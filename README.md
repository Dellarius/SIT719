# SIT719 - Analytics For Security And Privacy

This is my journel for work completed in SIT719 - Analytics For Security And Privacy

## Content
### Week 1
- [x] Task 1.1P - Understanding the Unit
- [x] Content - Fundamentals of Data Analytics using Python: Part 1
- [x] Seminar 1 Tasks
- [x] Practical 1 - Python Intro

### Week 2
- [X] Task 2.1P - Basic Scripting with Python
- [ ] Content - Fundamentals of Data Analytics using Python: Part 2
- [ ] Seminar 2 Tasks
- [ ] Practical 2, Task 1 - Programming Python using numpy, pandas
- [ ] Practical 2, Task 2 - NumPy's Structured Arrays
- [ ] Tutorial - Numpy Tutorial used in Class
- [ ] Tutorial - Pandas Tutorial used in Class

### Week 3
- [ ] Task 3.1P - Scikit-Learn for Machine Learning
- [ ] Task 3.2HD - Project Plan
- [ ] Content - Machine Learning for Cyber-Security
- [ ] Seminar 3 Tasks
- [ ] Pracical 3 Tasks - Supervised ML - Fraud & Credit Card Data detection

### Week 4
- [ ] Task 4.1P - Attack Classification using Naïve Bayes Algorithm
- [ ] Task 4.2C - Intrusion Detection using Supervised Learning Techniques
- [ ] Content - Supervised Machine Learning for Cyber-Security: Part 1
- [ ] Seminar 4 Tasks 
- [ ] Practical 4 - Task 1 - Experiment with WEKA

